...
//  SmartTalk MVP
//
//  Created by Pavel Mac on 8/22/24.
...
class SettingsViewController: UIViewController {
    ...
    private func initVM() {
        vm.loadInfoClosure = { [weak self] in
            ...
            let status = AprSdkDefaultSettings.shared.availability()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section == 4 && indexPath.row == 0 {
            ...
            alert.addAction(.init(title: "Sign Out", style: .destructive, handler: { [weak self](_) in
                guard let self = self else { return }
                ApiUsersAuthentication.shared.signOutUser { (isSuccess, error) in
                    if error != nil {
                        AprSdkAlert.showAlert(at: self, title: "Sign Out Validation", message: error!)
                    } else {
            ...
            AprSdkAlert.showAlert(at: self, title: "We will add it soon, Stay tune", message: "")
        }
    }
}
