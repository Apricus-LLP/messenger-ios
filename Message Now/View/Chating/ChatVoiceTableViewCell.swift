...
class ChatVoiceTableViewCell: UITableViewCell {
    ...
    var msgVirtualM   = MessageViewModel() { didSet{
        let seconds     = stringFormate(time: Int(msgVirtualM.voiceSec!)%60)
        let minuts      = stringFormate(time: Int(msgVirtualM.voiceSec!)/60)
        timeLabel.text  = "\(minuts):\(seconds)"
        timestampLabel.text = msgVirtualM.timestamp?.getMessageTime()
        seenImage.image   = msgVirtualM.isSeen! ? UIImage(named: "Seen") : UIImage(named: "Unseen")
        }}
    ...
    private func downloadFileFromURL(){
        let url = URL(string: msgVirtualM.voiceURL!)
