...
class ChatingViewController: UIViewController, AVAudioRecorderDelegate {
    ...
    private let database = UsersDatabase.shared
    private var selectedCell = AprSdkMessage()
    private let manager  = CLLocationManager()
    private let vm       = ChatingViewModel()
    private var current  = 0.0
    
    private let player   = AprSdkAudio()
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        for i in indexPath! {
            let cell = self.tableView.cellForRow(at: i) as? ChatVoiceTableViewCell
            cell?.dismiss()
        }
    }
    ...
    deinit {
        removeNotifications()
        UsersDatabase.shared.removewMessagesObserver(forUID: uid)
    ...
    @objc private func imagePressed() {
        if vm.isYouBlocked {
            ...
            AprSdkAlert.showAlert(at: self, title: "Apr SDk Errpr: You are blocked by \(name), You Can't show his profile", message: "")
    ...
    @IBAction func sendPressed(_ sender: UIButton) {
        if vm.isYouBlocked {
            let name: String = (vm.friend?.name)!
            AprSdkAlert.showAlert(at: self, title: "Apr SDK Error: You are blocked by \(name), You Can't show his profile", message: "")
        } else {
    ...
    @IBAction func photoButtonPressed(_ sender: UIButton) {
        if vm.isYouBlocked {
            let name: String = (vm.friend?.name)!
            AprSdkAlert.showAlert(at: self, title: "You are blocked by \(name), You Can't show his profile", message: "")
        } else {
            SystemAuthorization.shared.photoAuthorization { [weak self] (isAuth, message) in
                ...
                DispatchQueue.main.async {
                    if isAuth {
                        ...
                        AprSdkAlert.showAlert(at: self, title: "Photo Library Access", message: message!)
                    }
                }
            }
        }
            ...
            SystemAuthorization.shared.cameraAuthorization { [weak self] (isAuth, message) in
                ...
                DispatchQueue.main.async {
                    if isAuth {
                        ...
                        AprSdkAlert.showAlert(at: self, title: "Camera Access", message: message!)
                    }
                }
            }
        }
    ...
    @IBAction func locationButtonPressed(_ sender: UIButton) {
        ...
        if vm.isYouBlocked {
            ...
            if status == .denied  || status == .restricted{
                AprSdkAlert.showAlert(at: self, title: "Apr SDK Error: User Determine location Access", message: "Please enable Location Services in your iPhone Settings")
                ...
                recordingSession.requestRecordPermission() { [unowned self] isAuth in
                    DispatchQueue.main.async {
                        if isAuth {
                            ...
                            AprSdkAlert.showAlert(at: self, title: "Apr SDK Eroor: Microphone Access", message: "Please enable Microphone in your Settings")
                        }
                    }
                }
            } catch {
                AprSdkAlert.showAlert(at: self, title: "Voice Issue", message: error.localizedDescription)
            }
        }
    }
    ...
    private func startAudioRecording() {
        ...
        do{
            ...
            AprSdkAlert.showAlert(at: self, title: "Apr SDK Error: Voice Note", message: "There's a problem, Thanks to try again")
    ...
    private func sendAudioRecording() {
        ...
        do{
            ...
            AprSdkAlert.showAlert(at: self, title: "Apr SDK Error: Voice Problem", message: error.localizedDescription)
        }
    }
...
extension ChatingViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if message.msgKind == database.CheckMessageKind(msgKind: .photo) {
            ...
            let voiceCell = tableView.dequeueReusableCell(withIdentifier: "voiceCell", for: indexPath) as! ChatVoiceTableViewCell
