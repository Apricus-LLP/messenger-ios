...
class ChatsViewController: UIViewController, PresentChatingDelegate {
    ...
    private func initView() {
        ...
        AprSdkActivityIndicator.initActivityIndecator(view: view)
        AprSdkActivityIndicator.startAnimating()
    ...
    private func initVM() {
        vm.updateIndicatorClouser = { [weak self] in
            ...
            if !self.vm.isAnimating {
                AprSdkActivityIndicator.stopAnimating()
            } else {
                AprSdkActivityIndicator.startAnimating()
        ...
        vm.updateRequestsClouser = { [weak self] in
            guard let self = self else { return }
            let numberOfRequests = ApiNetworkRequest.shared.requestsRecived.count
