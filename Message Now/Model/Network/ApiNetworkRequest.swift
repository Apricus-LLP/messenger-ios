...
//  SmartTalk MVP
//
//  Created by Pavel Mac on 8/17/24.
...
class ApiNetworkRequest {
    ...
    public static let shared       = ApiNetworkRequest()
    
    private let REQ_REF            = ApiUsersAuthentication.shared.ref.child("request_friends")
    private let BLOCK_REF          = ApiUsersAuthentication.shared.ref.child("blocked_list")
    ...
    func confirmRequestFriend(uid: String) {
        ...
        ApiUsersAuthentication.shared.ref.child("friends_list").child(id).updateChildValues([uid: "true"])
        ApiUsersAuthentication.shared.ref.child("friends_list").child(uid).updateChildValues([id: "true"])
    }
    ...
    func loadUsersOfRequests(requestType: AprSdkRequestType, complation: @escaping([User]?)->()) {
        ...
        if keys.isEmpty {
            ...
            var index = 0
            for key in keys {
                UsersDatabase.shared.loadUserInfo(for: key) { (user, error) in
                    index += 1
                    users.append(user!)
                    if index == keys.count {
                        complation(users)
    ...
    func loadFriendsList(complation: @escaping([String])->()) {
        guard let id = currentUser.id else { return }
        ApiUsersAuthentication.shared.ref.child("friends_list").child(id).observe(.value) { [unowned self] (snapshot) in
            if snapshot.exists() {
    ...
    func loadFriends(complation: @escaping([User]?)->()) {
        ...
        if frinedsList.isEmpty {
            ...
            for key in frinedsList {
                UsersDatabase.shared.loadUserInfo(for: key) { [unowned self] (user, error) in
                    index += 1
                    users.append(user!)
    ...
    func removeFriend(uid: String) {
        guard let id = currentUser.id else { return }
        ApiUsersAuthentication.shared.ref.child("friends_list").child(id).child(uid).removeValue()
        ApiUsersAuthentication.shared.ref.child("friends_list").child(uid).child(id).removeValue()
    }
    ...
    func fetchBlockedUsers(complation: @escaping([User]?)->()) {
        ...
        if blockedList.isEmpty {
            ...
            for key in blockedList {
                UsersDatabase.shared.loadUserInfo(for: key) { [weak self] (user, error) in
                    guard let self = self else { return }
                    users.append(user!)
                    index += 1
